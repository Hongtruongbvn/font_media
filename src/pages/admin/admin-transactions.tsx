...
const AdminTransactionsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      try {
        ...
        if (!token) {
          setError(
            "Không tìm thấy mã thông báo xác thực. Vui lòng đăng nhập lại."
          );
          setLoading(false);
          return;
        }
        ...
        setError(
          err.response?.data?.message || "Đã xảy ra lỗi khi tải giao dịch."
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleDownloadReceipt = async (orderId: string) => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/payments/receipt/${orderId}`,
        {
